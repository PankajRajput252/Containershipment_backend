...
import com.sgmprime.model.DepositFund;
...
//    }
    ...
    public void confirmDeposit(String paymentId, String txHash) {
        ...
        // Update user wallet
        UserWallet wallet = userWalletRepository.findByUserFkId( deposit.getUserNodeId());

        if (Util.isDefined(wallet)) {
            wallet.setAvailableBalance(wallet.getAvailableBalance().add(deposit.getAmount()));
            userWalletRepository.save(wallet);

        } else {
            UserWallet userWallet = new UserWallet(deposit.getUserNodeId(),  BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO);
            userWallet.setAvailableBalance(BigDecimal.ZERO.add(deposit.getAmount()));
            userWalletRepository.save(userWallet);
        }

//      FinalResponse finalResponse=  confirmDeposit(paymentId);
